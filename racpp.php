...
/*  Jean-Philippe Policieux (Email : policieuxjp@gmail.com)
  ...
  This program is free software; you can redistribute it and/or pomodify
...
{  
	...
	add_meta_box( 'racpp-meta-box-id', 'Redirect after comment per page', 'racpp_meta_box', $post_type, 'normal', 'high' );  
    }
}  
...
if ($settings['enabled'] != '') {
    add_filter('comment_post_redirect', 'racpp_do_comment_redirect');
    add_action('comment_form', 'racpp_insert_referer');
}

function racpp_do_comment_redirect($location) {
    if(isset($_POST["comment_post_ID"])) $post_id=$_POST["comment_post_ID"];
